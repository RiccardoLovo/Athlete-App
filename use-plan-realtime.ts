...
export function usePlanRealtime(planId?: string | null) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "training_blocks" }, () => {
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "session_exercises" }, (payload) => {
        ...
        if (sid && payload.eventType !== "UPDATE") {
          qc.invalidateQueries({ queryKey: ["session-exs", sid], refetchType: "active" });
        }
        // Block-level count map is updated optimistically by the mutating
        // client; don't refetch it from realtime to avoid storms.
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "prescription_intervals" }, (payload) => {
        const seId =
          (payload.new as { session_exercise_id?: string } | null)?.session_exercise_id ??
          (payload.old as { session_exercise_id?: string } | null)?.session_exercise_id;
        if (seId) {
          qc.invalidateQueries({ queryKey: ["prescription-intervals", seId], refetchType: "active" });
        }
      })
      .subscribe();
    ...
    function onFocus() {
      ...
      qc.invalidateQueries({ queryKey: ["plan-blocks", planId] });
      qc.invalidateQueries({ queryKey: ["block-sessions"] });
      qc.invalidateQueries({ queryKey: ["session-exs"], refetchType: "active" });
      qc.invalidateQueries({ queryKey: ["block-session-ex-counts"], refetchType: "active" });
      qc.invalidateQueries({ queryKey: ["session-ctx"] });
    }
