...
function AuthPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      navigate({ to: role?.isAthlete ? "/dashboard" : "/exercises", replace: true });
    });
  }, [navigate]);
  ...
  async function signUp(e: React.FormEvent) {
    ...
    // Wait for session to be persisted before navigating
    await supabase.auth.getSession();
    toast.success("Account created. You're signed in — we'll remember you next time.");
    const role = await fetchMyRole();
    navigate({ to: role?.isAthlete ? "/dashboard" : "/exercises", replace: true });
  }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-background to-muted px-4">
      <Card className="w-full max-w-md p-8">
        <h1 className="text-center text-2xl font-bold text-primary">CoachDesk</h1>
            ...
            <form onSubmit={signUp} className="space-y-4 pt-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Use a strong, unique password. Passwords found in known breaches (like "Ricky97") are rejected.
                </p>
              </div>
              <Button type="submit" className="w-full" disabled={loading}>
              ...
              <p className="text-center text-xs text-muted-foreground">
                Athletes don't sign up here — your coach sends you a personal invite link.
              </p>
            </form>
          </TabsContent>
        </Tabs>
