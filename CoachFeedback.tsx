...
export function CoachFeedback({ embedded }: { embedded?: boolean } = {}) {
  ...
  if (selectedId) {
    return <FeedbackDetail id={selectedId} onBack={() => { setSelectedId(null); qc.invalidateQueries({ queryKey: ["workout-logs"] }); qc.invalidateQueries({ queryKey: ["pending-feedback-count"] }); }} />;
  }
  ...
  const inner = (
    <>
        <div className="mb-4 flex items-center justify-between">
          ...
          {pendingCount > 0 && <Badge className="bg-red-600 text-white">{pendingCount} pending</Badge>}
        </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-muted-foreground">
                        {l.sessions?.name || ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"][((l.sessions?.day_of_week ?? 1) - 1)]}
                      </span>
                      <Badge variant="secondary">W{l.sessions?.week_number}</Badge>
                    </div>
                    {l.overall_notes && <p className="mt-1 line-clamp-1 text-sm text-muted-foreground">{l.overall_notes}</p>}
                    <p className="mt-1 text-xs text-muted-foreground">{new Date(l.submitted_at).toLocaleString()}</p>
                  </div>
                  <Badge className={borgColor(l.borg_scale)}>{l.borg_scale}/10</Badge>
                  {l.status === "pending" ? <Clock className="h-5 w-5 text-yellow-600" /> : <Check className="h-5 w-5 text-emerald-600" />}
                </div>
              </Card>
            ))}
          </div>
