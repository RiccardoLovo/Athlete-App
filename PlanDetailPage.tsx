...
import {
  blockStart, blockEnd, planEnd, parseISODate, formatRange, isBetween,
  toISODate,
...
export function PlanDetailPage() {
  ...
  async function addBlock() {
    const nextPos = blocks.length ? Math.max(...blocks.map((b) => b.position)) + 1 : 1;
    const { error } = await supabase
      .from("training_blocks")
      .insert({ plan_id: planId, name: `Block ${nextPos}`, position: nextPos, weeks: 4 });
    if (error) return toast.error(error.message);
    invalidate();
  }

  async function updateBlock(id: string, patch: Partial<Block>) {
    const { error } = await supabase.from("training_blocks").update(patch).eq("id", id);
  ...
  async function moveBlock(b: Block, dir: -1 | 1) {
    ...
    await supabase.from("training_blocks").update({ position: -Math.abs(b.position) }).eq("id", b.id);
    await supabase.from("training_blocks").update({ position: b.position }).eq("id", swap.id);
    await supabase.from("training_blocks").update({ position: swap.position }).eq("id", b.id);
    invalidate();
  }

  async function updatePlanStart(newStart: string) {
    if (!newStart || newStart === plan.start_date) return;
    const { error } = await supabase.from("training_plans").update({ start_date: newStart }).eq("id", planId);
    if (error) return toast.error(error.message);
  ...
  async function updateBlockEnd(b: Block, newEndISO: string) {
    ...
    if (weeks === b.weeks) {
      toast.info("End date snapped to current week count (blocks are whole-week sized).");
      return;
    }
  ...
  async function updateBlockStart(b: Block, newStartISO: string) {
    ...
    const days = Math.round((newStart.getTime() - prevStart.getTime()) / 86_400_000);
  ...
  const todayPct = todayInPlan ? ((today.getTime() - start.getTime()) / 86_400_000 / totalDays) * 100 : null;

  return (
    <div className="h-[calc(100vh-3.5rem)] overflow-auto">
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <nav className="text-xs text-muted-foreground">
          <Link to="/clients/$clientId" params={{ clientId: plan.athlete_id }} className="hover:text-foreground">{plan.clients?.name}</Link>
          <span> / </span><span>{plan.name}</span>
        </nav>
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate({ to: "/clients/$clientId", params: { clientId: plan.athlete_id } })}>
          ...
          <div className="flex-1">
            ...
            <div className="text-sm text-muted-foreground">
              {formatRange(start, end)} · {totalWeeks} week{totalWeeks === 1 ? "" : "s"}
            </div>
          </div>
        </div>
        ...
        <section>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Button onClick={addBlock}><Plus className="mr-1 h-4 w-4" /> Add Block</Button>
          </div>
          {blocks.length === 0 ? (
                ...
                return (
                  <Card key={b.id} className="p-3">
                    <div className="flex flex-wrap items-center gap-3">
                      <div className={`h-8 w-2 rounded ${BLOCK_COLORS[i % BLOCK_COLORS.length]}`} />
                      <Input
                        defaultValue={b.name}
                        onBlur={(e) => { if (e.target.value !== b.name) updateBlock(b.id, { name: e.target.value }); }}
                        className="h-8 max-w-[260px] font-semibold"
                      />
                      <div className="flex items-center gap-1 text-xs">
                        <span className="text-muted-foreground">Weeks</span>
                        <Input
                          type="number" min={1} max={52} defaultValue={b.weeks}
                          onBlur={(e) => {
                            const w = parseInt(e.target.value, 10);
                            if (Number.isFinite(w) && w > 0 && w !== b.weeks) updateBlock(b.id, { weeks: w });
                          }}
                          className="h-8 w-16"
                        />
                      </div>
                      <div className="flex items-center gap-1 text-xs">
                        ...
                        <Input
                          ...
                          type="date" defaultValue={toISODate(bs)}
                      ...
                      <div className="flex items-center gap-1 text-xs">
                        ...
                        <Input
                          ...
                          type="date" defaultValue={toISODate(be)}
                          onBlur={(e) => updateBlockEnd(b, e.target.value)}
                          className="h-8 w-[140px]"
                        />
                      </div>
                      <div className="ml-auto flex items-center gap-1">
                        <Link to="/plans/$planId/blocks/$blockId" params={{ planId, blockId: b.id }}>
                          <Button size="sm" variant="outline">Open</Button>
                        </Link>
                        <Button size="icon" variant="ghost" disabled={i === 0} onClick={() => moveBlock(b, -1)}>
                          <ChevronUp className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" disabled={i === blocks.length - 1} onClick={() => moveBlock(b, 1)}>
