...
import { addDays, parseISODate, toISODate, formatLong } from "@/lib/coachdesk/periodization";
...
export function MyFeedback({ wrap }: { wrap?: boolean } = {}) {
  ...
  const { data: sessions = [], isLoading } = useQuery({
    ...
    queryFn: async (): Promise<PlanSession[]> => {
      const { data: plans } = await supabase
        .from("training_plans")
        .select("id, start_date, status, training_blocks(id, name, position, weeks)")
      ...
      for (const plan of (plans ?? []) as any[]) {
        ...
        const [{ data: exs }, { data: logs }] = await Promise.all([
          supabase.from("session_exercises").select("session_id, exercises(discipline, category)").in("session_id", sIds),
          supabase.from("workout_logs").select("id, session_id, borg_scale, overall_notes, submitted_at, performed_at, status")
            .eq("client_id", client!.id).in("session_id", sIds),
        ]);
        ...
        for (const e of (exs ?? []) as any[]) {
          ...
          if (t) {
            if (!typesMap.has(e.session_id)) typesMap.set(e.session_id, new Set());
        ...
        let cum = 0;
        for (const b of blocks) { offsets.set(b.id, cum); cum += b.weeks * 7; }
        ...
        for (const s of sList) {
          ...
          if (!blk) continue;
          const off = (offsets.get(s.block_id) ?? 0) + (s.week_number - 1) * 7 + (s.day_of_week - 1);
          const planned = toISODate(addDays(parseISODate(plan.start_date), off));
  ...
  const content = (() => {
    if (!client) {
      return <div className="py-20 text-center text-muted-foreground">You don't have an athlete profile linked to this account.</div>;
    }
    if (openId) {
      const s = sessions.find((x) => x.id === openId);
      if (!s) return <div className="py-20 text-center text-muted-foreground">Session not found.</div>;
      return <MyFeedbackDetail client={client} session={s} onBack={() => setOpenId(null)} />;
    }
    return (
      <>
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-2xl font-bold">My Feedback</h1>
          <p className="text-sm text-muted-foreground">Log or edit feedback on your past sessions.</p>
        </div>
        {isLoading ? (
          ...
          <div className="space-y-2">
            {visible.map((s) => {
              const logged = !!s.log;
              const label = s.name || `Training Day ${s.day_of_week} - Week ${s.week_number}`;
              const performed = s.log?.performed_at ?? null;
              const dateDisplay = formatLong(parseISODate(performed ?? s.planned_date));
              return (
                <Card
                  key={s.id}
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground inline-flex items-center gap-1">
                          ...
                          {performed && performed !== s.planned_date && (
                            <span className="text-[10px] italic">(planned {formatLong(parseISODate(s.planned_date))})</span>
                          )}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-muted-foreground">{s.ex_count} exercise{s.ex_count === 1 ? "" : "s"}</p>
                      {s.log?.overall_notes && (
                        <p className="mt-1 line-clamp-1 text-sm text-muted-foreground">{s.log.overall_notes}</p>
                      )}
                    </div>
                    {logged ? (
                      <>
                        <Badge className={borgColor(s.log!.borg_scale)}>{s.log!.borg_scale}/10</Badge>
