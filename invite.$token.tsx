...
function InvitePage() {
  ...
  const { data: invite, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return row as { valid: boolean; used: boolean; client_name: string; coach_name: string } | null;
    },
  });
      ...
      <div className="flex min-h-screen items-center justify-center bg-muted/30 px-4">
        <Card className="max-w-md p-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {invite?.used ? "This invite has already been used." : "This invite link isn't valid. Ask your coach for a new one."}
          </p>
        </Card>
      </div>
    );
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-background to-muted px-4">
      <Card className="w-full max-w-md p-8">
        <h1 className="text-center text-2xl font-bold text-primary">Welcome to CoachDesk</h1>
        ...
        <form onSubmit={signUp} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Use a strong, unique password (8+ characters).</p>
          </div>
