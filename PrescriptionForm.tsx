...
import {
  type Discipline, type Prescription, emptyPrescription, summarizePrescription,
...
export function PrescriptionForm({
  rowId, discipline, initial, oneRmKg, strokeDefault, onChanged,
  structureType = "simple", templateType = null, templateDefaults = null, templateGeneratedAt = null,
}: {
  rowId: string;
  ...
  useEffect(() => {
    if (discipline === "Swimming" && strokeDefault && !form.prescription.stroke) {
  ...
  const flush = useCallback(async () => {
    ...
    const payload = {
      sets: next.sets, reps: next.reps,
      target_mode: next.target_mode, rpe: next.rpe,
      load_mode: next.load_mode, load_value: next.load_value,
      rest_sec: next.rest_sec,
      distance_km: next.distance_km, duration_min: next.duration_min,
      pace: next.pace, hr_zone: next.hr_zone,
      tempo: next.tempo, notes: next.notes,
      prescription: next.prescription,
    };

    const { error } = await supabase.from("session_exercises").update(payload as never).eq("id", rowId);
  ...
  useEffect(() => {
    function onPageHide() { beaconSave(); void flush(); }
    function onVisibilityChange() {
      if (document.visibilityState === "hidden") { beaconSave(); void flush(); }
    }
    function onBeforeUnload() { beaconSave(); void flush(); }
  ...
  function patchJson(k: string, v: unknown) {
    ...
    if (v === null || v === "" || v === undefined) delete j[k]; else j[k] = v;
  ...
  const body = (() => {
    switch (discipline) {
      case "Strength":   return <StrengthForm form={form} patch={patch} patchJson={patchJson} oneRmKg={oneRmKg} />;
      case "Running":    return <RunningForm form={form} patch={patch} patchJson={patchJson} />;
      case "Swimming":   return <SwimmingForm form={form} patch={patch} patchJson={patchJson} />;
      case "Cycling":    return <CyclingForm form={form} patch={patch} patchJson={patchJson} />;
      case "Sport-Specific": return <SportForm form={form} patch={patch} />;
      case "Mobility":   return <MobilityForm form={form} patch={patch} patchJson={patchJson} />;
    }
  })();
...
function NumIn({ value, onChange, placeholder, step }: {
  value: number | null; onChange: (n: number | null) => void; placeholder?: string; step?: string;
}) {
  return (
    <Input
      type="number"
...
function TextIn({ value, onChange, placeholder }: {
  ...
  return (
    <Input className="h-7" value={value ?? ""} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} />
  );
}

function Sel({ value, onChange, options, allowEmpty }: {
  value: string | null;
...
function NotesRpe({ form, patch }: { form: Prescription; patch: (p: Partial<Prescription>) => void }) {
  return (
    <>
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("common.rpe")} (1-10)</L><NumIn value={form.rpe} onChange={(n) => patch({ rpe: n })} /></div>
      </div>
      <div><L>{t("common.notes")}</L><Textarea rows={2} value={form.notes} onChange={(e) => patch({ notes: e.target.value })} /></div>
    </>
  );
}

function IntervalsSection({
  rowId, discipline, structureType, templateType, templateDefaults, templateGeneratedAt,
  hasRows, form, patch, status, flush, onChanged,
}: {
  rowId: string;
  discipline: Discipline;
  structureType: "simple" | "intervals" | "template";
...
function StrengthForm({ form, patch, patchJson, oneRmKg }: FormProps) {
  ...
  const calcKg = isPct && oneRmKg && pct != null ? Math.round((pct / 100) * oneRmKg * 10) / 10 : null;
  const rir = form.prescription.rir;
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("common.sets")}</L><NumIn value={form.sets} onChange={(n) => patch({ sets: n })} /></div>
        <div><L>{t("common.reps")}</L><TextIn value={form.reps} onChange={(s) => patch({ reps: s })} placeholder="8-10" /></div>
      </div>
      <div>
        ...
        <div className="flex gap-1">
          {(["kg", "%1RM", "bodyweight"] as const).map((m) => (
            <button
              key={m}
              type="button"
              onClick={() => patch({ load_mode: m, load_value: m === "bodyweight" ? null : form.load_value })}
      ...
      {!isBw && (
        <div>
          ...
          <NumIn value={form.load_value} onChange={(n) => patch({ load_value: n })} placeholder={isPct ? "75" : "80"} step="0.5" />
          {isPct && pct != null && (
            oneRmKg != null
              ? <div className="mt-1 text-[10px] text-muted-foreground">{t("strength.pct_calc", { pct: pct, oneRm: oneRmKg, abs: calcKg ?? "?" })}</div>
              : <div className="mt-1 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] text-amber-800 dark:bg-amber-500/15 dark:text-amber-300">{t("strength.no_1rm")}</div>
          )}
        </div>
      )}
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("common.rest_sec")}</L><NumIn value={form.rest_sec} onChange={(n) => patch({ rest_sec: n })} /></div>
        <div><L>{t("common.tempo")}</L><TextIn value={form.tempo} onChange={(s) => patch({ tempo: s })} placeholder={t("strength.tempo.placeholder")} /></div>
      </div>
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("common.rpe")} (1-10)</L><NumIn value={form.rpe} onChange={(n) => patch({ rpe: n })} /></div>
        <div><L>{t("common.rir")}</L><NumIn value={typeof rir === "number" ? rir : null} onChange={(n) => patchJson?.("rir", n)} /></div>
      </div>
      <div><L>{t("common.notes")}</L><Textarea rows={2} value={form.notes} onChange={(e) => patch({ notes: e.target.value })} /></div>
    </div>
  );
...
function RunningForm({ form, patch }: FormProps) {
  ...
  return (
    <div className="space-y-2">
      <div><L>{t("target.mode")}</L><Sel value={form.target_mode} onChange={(v) => patch({ target_mode: v })} options={["Distance", "Time"]} /></div>
      {isDist && <div><L>{t("field.distance_km")}</L><NumIn value={form.distance_km} onChange={(n) => patch({ distance_km: n })} step="0.1" /></div>}
      {isTime && <div><L>{t("field.duration_min")}</L><NumIn value={form.duration_min} onChange={(n) => patch({ duration_min: n })} step="0.5" /></div>}
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("field.pace_min_km")}</L><TextIn value={form.pace} onChange={(s) => patch({ pace: s })} placeholder="4:30" /></div>
        <div><L>{t("field.hr_zone")}</L><Sel value={form.hr_zone == null ? null : String(form.hr_zone)} onChange={(v) => patch({ hr_zone: v ? Number(v) : null })} options={["1","2","3","4","5"]} allowEmpty /></div>
      </div>
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("field.reps_intervals")}</L><NumIn value={intervalReps ?? null} onChange={(n) => patchJson("interval_reps", n)} /></div>
        <div><L>{t("common.rest_sec")}</L><NumIn value={form.rest_sec} onChange={(n) => patch({ rest_sec: n })} /></div>
      </div>
      <NotesRpe form={form} patch={patch} />
    </div>
...
function SwimmingForm({ form, patch, patchJson }: FormProps) {
  ...
  return (
    <div className="space-y-2">
      <div><L>{t("target.mode")}</L><Sel value={form.target_mode} onChange={(v) => patch({ target_mode: v })} options={["Distance","Time","Laps"]} /></div>
      {isDist && <div><L>{t("field.distance_m")}</L><NumIn value={form.distance_km == null ? null : Math.round(form.distance_km * 1000)} onChange={(n) => patch({ distance_km: n == null ? null : n / 1000 })} /></div>}
      {isLaps && <div><L>{t("field.laps")}</L><NumIn value={laps ?? null} onChange={(n) => patchJson?.("laps", n)} /></div>}
      {isTime && <div><L>{t("field.duration_min")}</L><NumIn value={form.duration_min} onChange={(n) => patch({ duration_min: n })} step="0.5" /></div>}
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("field.stroke")}</L><Sel value={stroke} onChange={(v) => patchJson?.("stroke", v)} options={STROKES} allowEmpty /></div>
        <div><L>{t("field.pace_min_100m")}</L><TextIn value={form.pace} onChange={(s) => patch({ pace: s })} placeholder="1:45" /></div>
      </div>
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("field.hr_zone")}</L><Sel value={form.hr_zone == null ? null : String(form.hr_zone)} onChange={(v) => patch({ hr_zone: v ? Number(v) : null })} options={["1","2","3","4","5"]} allowEmpty /></div>
        <div><L>{t("field.equipment")}</L><Sel value={equip} onChange={(v) => patchJson?.("equipment", v)} options={EQUIPMENT} allowEmpty /></div>
      </div>
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("field.reps_intervals")}</L><NumIn value={intervalReps ?? null} onChange={(n) => patchJson?.("interval_reps", n)} /></div>
        <div><L>{t("common.rest_sec")}</L><NumIn value={form.rest_sec} onChange={(n) => patch({ rest_sec: n })} /></div>
...
function CyclingForm({ form, patch, patchJson }: FormProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      {isTime && <div><L>{t("field.duration_min")}</L><NumIn value={form.duration_min} onChange={(n) => patch({ duration_min: n })} step="0.5" /></div>}
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("field.watts")}</L><NumIn value={watts ?? null} onChange={(n) => patchJson?.("watts", n)} /></div>
        <div><L>{t("field.cadence_rpm")}</L><NumIn value={cadence ?? null} onChange={(n) => patchJson?.("cadence_rpm", n)} /></div>
      </div>
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("field.hr_zone")}</L><Sel value={form.hr_zone == null ? null : String(form.hr_zone)} onChange={(v) => patch({ hr_zone: v ? Number(v) : null })} options={["1","2","3","4","5"]} allowEmpty /></div>
        <div><L>{t("field.reps_intervals")}</L><NumIn value={intervalReps ?? null} onChange={(n) => patchJson?.("interval_reps", n)} /></div>
...
function SportForm({ form, patch }: FormProps) {
  ...
  return (
    <div className="space-y-2">
      <div><L>{t("target.mode")}</L><Sel value={form.target_mode} onChange={(v) => patch({ target_mode: v })} options={["Time","Reps"]} /></div>
      {isTime && <div><L>{t("field.duration_min")}</L><NumIn value={form.duration_min} onChange={(n) => patch({ duration_min: n })} step="0.5" /></div>}
      {isReps && <div><L>{t("common.reps")}</L><TextIn value={form.reps} onChange={(s) => patch({ reps: s })} placeholder="10" /></div>}
...
function MobilityForm({ form, patch, patchJson }: FormProps) {
  ...
  return (
    <div className="space-y-2">
      <div><L>{t("target.mode")}</L><Sel value={form.target_mode} onChange={(v) => patch({ target_mode: v })} options={["Hold","Reps"]} /></div>
      {isHold && <div><L>{t("field.hold_sec")}</L><NumIn value={hold ?? null} onChange={(n) => patchJson?.("hold_sec", n)} /></div>}
      {isReps && <div><L>{t("common.reps")}</L><TextIn value={form.reps} onChange={(s) => patch({ reps: s })} placeholder="10" /></div>}
      <div className="grid grid-cols-2 gap-1">
        <div><L>{t("common.sets")}</L><NumIn value={form.sets} onChange={(n) => patch({ sets: n })} /></div>
        <div><L>{t("common.side")}</L><Sel value={side} onChange={(v) => patchJson?.("side", v)} options={SIDES} allowEmpty /></div>
      </div>
      <div><L>{t("common.notes")}</L><Textarea rows={2} value={form.notes} onChange={(e) => patch({ notes: e.target.value })} /></div>
    </div>
  );
...
export function rowToPrescription(row: Record<string, unknown>): Prescription {
  const get = <T,>(k: string): T | null => (row[k] as T | null | undefined) ?? null;
  return {
    ...emptyPrescription(),
