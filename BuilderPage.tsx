...
import {
  ArrowLeft, Dumbbell, GripVertical, Trash2, ChevronDown, ChevronRight, Tag, X, Plus, ArrowUp, ArrowDown,
} from "lucide-react";
...
import {
  TRAINING_CATEGORIES, BODY_REGIONS, trainingCategoryMeta, bodyRegionLabel, BODY_REGION_BADGE,
  type TrainingCategoryKey, type BodyRegionKey,
} from "@/lib/coachdesk/tags";
import {
  type Discipline, DISCIPLINES, DISCIPLINE_BADGE, DISCIPLINE_ICON,
  defaultForDiscipline, summarizePrescription,
...
import {
  DAYS_OF_WEEK, DAYS_OF_WEEK_LONG, parseISODate, addDays, formatLong, blockStart,
...
type SE = {
  ...
  exercises: { name_en: string; discipline: Discipline; stroke_default: string | null };
};
...
export function BuilderPage() {
  ...
  const { data: ctx } = useQuery({
    ...
    queryFn: async () => {
      const { data: s } = await supabase
        .from("sessions")
        .select("id, name, notes, day_of_week, week_number, status, is_optional, training_category_tags, body_region, block_id, training_blocks(id, name, plan_id, weeks, position, training_plans(id, name, start_date, athlete_id, clients(id, name, sport, goal, coach_id)))")
        .eq("id", sessionId)
        .single();
      return s as any;
    },
  ...
  const planStart = ctx?.training_blocks?.training_plans?.start_date as string | undefined;
  ...
  const planName = ctx?.training_blocks?.training_plans?.name as string | undefined;
  const blockName = ctx?.training_blocks?.name as string | undefined;
  const client = ctx?.training_blocks?.training_plans?.clients as { id: string; name: string; sport: string; goal: string; coach_id: string } | undefined;

  const { data: siblingBlocks = [] } = useQuery({
    queryKey: ["plan-blocks", planId],
    enabled: !!planId,
    queryFn: async () => {
      const { data } = await supabase.from("training_blocks").select("position, weeks").eq("plan_id", planId!);
  ...
  const sessionDate = useMemo(() => {
    ...
    const bStart = blockStart(planStart, siblingBlocks, ctx.training_blocks.position);
  ...
  const { data: blockSessions = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data ?? []) as { id: string; week_number: number; day_of_week: number; name: string | null }[];
    },
  });
  ...
  async function jumpToDay(week: number, day: number) {
    const existing = blockSessions.find((s) => s.week_number === week && s.day_of_week === day);
    if (existing) {
      navigate({ to: "/builder/$sessionId", params: { sessionId: existing.id } });
      return;
    }
    const { data, error } = await supabase
      .from("sessions")
      .insert({ block_id: blockId!, week_number: week, day_of_week: day, status: "active" } as never)
      .select("id").single();
    if (error) return toast.error(error.message);
    if (data) navigate({ to: "/builder/$sessionId", params: { sessionId: data.id } });
  }
  ...
  async function onDropExercise(exerciseId: string) {
    ...
    const lastIndex = exs.length === 0 ? -1 : Math.max(...exs.map((s) => s.order_index));
    const { error } = await supabase.from("session_exercises").insert({
      session_id: sessionId, exercise_id: exerciseId, order_index: lastIndex + 1,
  ...
  async function updateSessionName(name: string) {
    const { error } = await supabase.from("sessions").update({ name }).eq("id", sessionId);
  ...
  async function toggleTag(key: TrainingCategoryKey) {
    ...
    const { error } = await supabase.from("sessions").update({ training_category_tags: next }).eq("id", sessionId);
    if (error) return toast.error(`Save failed: ${error.message}`);
    invalidate();
  }
  async function setRegion(key: BodyRegionKey | null) {
    const { error } = await supabase.from("sessions").update({ body_region: key }).eq("id", sessionId);
    if (error) return toast.error(`Save failed: ${error.message}`);
    invalidate();
  }
  ...
  useEffect(() => {
    function flushSessionName() {
      ...
      void supabase.from("sessions").update({ name: value }).eq("id", sessionId);
    }
    ...
    <div className="flex h-[calc(100vh-3.5rem)] flex-col">
      <div className="border-b bg-card px-4 py-3">
        <nav className="text-xs text-muted-foreground">
          {client && <Link to="/clients/$clientId" params={{ clientId: client.id }} className="hover:text-foreground">{client.name}</Link>}
          <span> / </span>
          {planId && <Link to="/plans/$planId" params={{ planId }} className="hover:text-foreground">{planName}</Link>}
          <span> / </span>
          {planId && blockId && (
            <Link to="/plans/$planId/blocks/$blockId" params={{ planId, blockId }} className="hover:text-foreground">
              {blockName}
            </Link>
          )}
          <span> / Week {ctx.week_number} · {dayLabel}</span>
        </nav>
        <div className="mt-2 flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate({ to: "/plans/$planId/blocks/$blockId", params: { planId: planId!, blockId: blockId! } })}>
          ...
          <div className="flex-1">
            ...
            />
            {sessionDate && <div className="text-xs text-muted-foreground">{formatLong(sessionDate)}</div>}
          </div>
          <Button variant="outline" onClick={() => setShowBank((v) => !v)}>{showBank ? "Hide" : "Show"} Bank</Button>
        </div>
        {blockId && totalWeeks > 0 && (
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            <span className="ml-2 text-[10px] font-semibold uppercase text-muted-foreground">Day</span>
            <div className="flex flex-wrap gap-1">
              {DAYS_OF_WEEK.map((lbl, i) => {
                const d = i + 1;
                const hasSession = blockSessions.some((s) => s.week_number === ctx.week_number && s.day_of_week === d);
                ...
                return (
                  ...
                  >
                    {lbl}{!hasSession && !isCurrent && <Plus className="h-2.5 w-2.5" />}
                  </button>
                );
              })}
            </div>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          {tags.map((tg) => {
            ...
            if (!meta) return null;
            return <span key={tg} className={`rounded-full border px-2 py-0.5 text-[11px] ${meta.className}`}>{meta.label}</span>;
          })}
          {region && <span className={`rounded-full border px-2 py-0.5 text-[11px] ${BODY_REGION_BADGE}`}>{bodyRegionLabel(region)}</span>}
          <Popover>
            ...
            <PopoverContent align="start" className="w-56 p-3">
              <div className="mb-1 text-[10px] font-semibold uppercase text-muted-foreground">Training category</div>
              ...
              <div className="mb-1 text-[10px] font-semibold uppercase text-muted-foreground">Body region</div>
              <div className="flex flex-wrap gap-1">
                {BODY_REGIONS.map((r) => {
                  const active = region === r.key;
                  return (
                    <button key={r.key} onClick={() => setRegion(active ? null : r.key)}
      ...
      <div className="flex flex-1 overflow-hidden">
        {showBank && (
          <aside className="w-[240px] shrink-0 overflow-auto border-r bg-muted/30 p-3">
            <div className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Drag to add</div>
            <Input placeholder="Search…" className="mb-2 h-8" value={bankSearch} onChange={(e) => setBankSearch(e.target.value)} />
              ...
              {BODY_REGIONS.map((r) => {
                ...
                return (
                  <button key={r.key} onClick={() => {
                    ...
                    active ? next.delete(r.key) : next.add(r.key);
            ...
            <div className="space-y-1">
              {filteredBank.map((e) => (
                <div
                  key={e.id}
                  draggable
                  onDragStart={(ev) => ev.dataTransfer.setData("application/json", JSON.stringify({ exerciseId: e.id }))}
            ...
            <div className="mt-2 text-xs text-muted-foreground">{filteredBank.length} exercises</div>
          </aside>
        )}
...
function ExerciseCard({ se, defaultExpanded, onChanged, oneRmKg, canMoveUp, canMoveDown, onMoveUp, onMoveDown, onOptimisticRemove }: {
  ...
  return (
    <div className="rounded border bg-card p-3 text-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="flex-1 truncate font-semibold">{se.exercises?.name_en}</span>
        <button
          onClick={onMoveUp}
          disabled={!canMoveUp}
          title="Move up"
        ...
        <button onClick={remove}><Trash2 className="h-4 w-4 text-destructive" /></button>
      </div>
      {!expanded ? (
