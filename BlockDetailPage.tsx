...
import { ArrowLeft, Plus, Copy, Trash2, Moon, Dumbbell, GripVertical, ChevronDown, ChevronRight } from "lucide-react";
...
import {
  blockStart, weekRange, formatShort, weekIndexForDate, DAYS_OF_WEEK,
} from "@/lib/coachdesk/periodization";
import { ExerciseBank, insertExerciseIntoSession } from "@/components/coachdesk/ExerciseBank";
import { SessionExercises } from "@/components/coachdesk/SessionExercises";
import { usePlanRealtime } from "@/hooks/use-plan-realtime";

type Session = {
  id: string; name: string | null; day_of_week: number; week_number: number; status: string;
  created_at: string; updated_at: string;
};

export function BlockDetailPage() {
  const { planId, blockId } = useParams({ from: "/_authenticated/plans/$planId/blocks/$blockId" });
  ...
  const { data: ctx } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("id, name, weeks, position, plan_id, training_plans(id, name, start_date, status, athlete_id, clients(id, name))")
  ...
  const bStart = useMemo(() => {
    if (!ctx) return null;
    return blockStart(ctx.training_plans.start_date, siblingBlocks, ctx.position);
  }, [ctx, siblingBlocks]);
  ...
  const { data: sessions = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("sessions")
        .select("id, name, day_of_week, week_number, status, created_at, updated_at")
        .eq("block_id", blockId)
  ...
  const { data: exCountMap, isFetched: countsFetched } = useQuery<Record<string, number>>({
  ...
  useEffect(() => {
    ...
    const weekHasSessions = (week: number) => sessions.some((s) => s.week_number === week);
  ...
  async function createSession(week: number, day: number, opts?: { silent?: boolean }) {
  ...
  async function moveSession(sessionId: string, newDay: number) {
    const { error } = await supabase.from("sessions").update({ day_of_week: newDay }).eq("id", sessionId);
    if (error) toast.error(error.message);
    invalidate();
  }

  async function addExerciseToDay(week: number, day: number, exerciseId: string) {
    const existing = sessions.filter((s) => s.week_number === week && s.day_of_week === day);
    ...
    if (error) { toast.error(`Add exercise failed: ${error.message}`); return; }
  ...
  async function handleDayDrop(week: number, day: number, ev: React.DragEvent) {
    ...
    if (sid) { moveSession(sid, day); return; }
    ...
    try { exerciseId = JSON.parse(raw).exerciseId; } catch { exerciseId = raw; }
  ...
  async function copyWeek(target: number | "all") {
    if (target === "all") {
      for (let w = 1; w <= totalWeeks; w++) {
        ...
        const { error } = await supabase.rpc("copy_week", { _block_id: blockId, _source_week: activeWeek, _target_week: w });
        if (error) { toast.error(error.message); return; }
      }
      toast.success(`Week ${activeWeek} copied to all other weeks`);
    } else {
      const { error } = await supabase.rpc("copy_week", { _block_id: blockId, _source_week: activeWeek, _target_week: target });
  ...
  async function toggleRest(s: Session) {
    ...
    const { error } = await supabase.from("sessions").update({ status: newStatus }).eq("id", s.id);
    if (error) return toast.error(`Save failed: ${error.message}`);
    invalidate();
  }

  if (!ctx || !bStart) return <div className="p-6 text-muted-foreground">Loading…</div>;
    ...
    <div className="flex h-[calc(100vh-3.5rem)] flex-col">
      <div className="border-b bg-card px-4 py-3">
        <nav className="text-xs text-muted-foreground">
          <Link to="/clients/$clientId" params={{ clientId }} className="hover:text-foreground">{ctx.training_plans.clients?.name}</Link>
          <span> / </span>
          <Link to="/plans/$planId" params={{ planId: planId_ }} className="hover:text-foreground">{ctx.training_plans.name}</Link>
          <span> / </span><span>{ctx.name}</span>
        </nav>
        <div className="mt-2 flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate({ to: "/plans/$planId", params: { planId: planId_ } })}>
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div className="flex-1">
            <h1 className="text-xl font-bold">{ctx.name}</h1>
            <div className="text-xs text-muted-foreground">Week {activeWeek} of {totalWeeks} · {formatShort(activeWeekRange.start)} – {formatShort(activeWeekRange.end)}</div>
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline"><Copy className="mr-1 h-4 w-4" /> Copy Week</Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              {Array.from({ length: totalWeeks }, (_, i) => i + 1).filter((w) => w !== activeWeek).map((w) => (
                <DropdownMenuItem key={w} onClick={() => copyWeek(w)}>Copy to Week {w}</DropdownMenuItem>
              ))}
              {totalWeeks > 2 && <DropdownMenuSeparator />}
              {totalWeeks > 1 && <DropdownMenuItem onClick={() => copyWeek("all")}>Copy to all remaining weeks</DropdownMenuItem>}
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
          ...
          {Array.from({ length: totalWeeks }, (_, i) => i + 1).map((w) => {
            ...
            return (
              ...
              >
                ...
                {isCurrent && <span className="ml-1 text-[9px] uppercase">today</span>}
              </button>
            );
          })}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {showBank && <ExerciseBank selectedExerciseId={selectedExercise?.id ?? null} onSelectExercise={setSelectedExercise} />}
        <div className="flex-1 overflow-auto p-4">
        <div className="mb-2 flex justify-end">
          <Button variant="outline" size="sm" onClick={() => setShowBank((v) => !v)}>
        ...
        <div className="flex flex-col gap-3">
          {DAYS_OF_WEEK.map((label, idx) => {
            const day = idx + 1; // 1..7
            const dayDate = new Date(activeWeekRange.start); dayDate.setDate(dayDate.getDate() + idx);
            const daySessions = sessions.filter((s) => s.week_number === activeWeek && s.day_of_week === day);
            return (
              <div
                key={day}
              ...
              >
                <div className="flex items-center justify-between border-b px-3 py-2 md:w-40 md:shrink-0 md:border-b-0 md:border-r">
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground">{formatShort(dayDate)}</div>
                  </div>
                </div>
                <div className="flex flex-1 flex-col gap-2 p-2">
                    ...
                    return (
                      <Card key={s.id} className="p-2">
                        <div className="flex items-center gap-1">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="start">
                              <div className="px-2 py-1 text-[10px] uppercase text-muted-foreground">Move to day</div>
                              {DAYS_OF_WEEK.map((dl, di) => {
                                const target = di + 1;
                                if (target === s.day_of_week) return null;
                                return (
                                  <DropdownMenuItem key={target} onClick={() => moveSession(s.id, target)}>
                                    {dl}
                                  </DropdownMenuItem>
                                );
                              })}
                          ...
                          {s.status === "rest" ? (
                            ...
                            >
                              ...
                              <span className="truncate text-xs font-semibold">{s.name || label}</span>
                            </button>
                          )}
                          <Link
                          ...
                          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => toggleRest(s)} title="Toggle rest">
                            <Moon className={`h-3 w-3 ${s.status === "rest" ? "fill-blue-400 text-blue-500" : "text-muted-foreground"}`} />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => deleteSession(s.id)}>
                        ...
                        {s.status !== "rest" && (
                          isExpanded ? (
                            <div className="mt-2">
                              <SessionExercises sessionId={s.id} clientId={clientId} compact />
                            </div>
                          ) : (
                            <button onClick={() => toggleSession(s.id)} className="mt-1 block w-full text-left">
                              <SessionExCount
                  ...
                  >
                    <Plus className="mr-1 h-3 w-3" /> {daySessions.length ? "Add another" : "Session"}
                  </button>
                </div>
              </div>
            );
...
function SessionExCount({ count, loaded }: { count: number | undefined; loaded: boolean }) {
  ...
  return <div className="mt-0.5 text-[10px] text-muted-foreground">{count} exercise{count === 1 ? "" : "s"}</div>;
}
