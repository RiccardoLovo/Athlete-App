...
export function blockStart(planStart: string, blocks: BlockLike[], blockPosition: number): Date {
...
export function blockEnd(planStart: string, blocks: BlockLike[], blockPosition: number): Date {
...
export function weekRange(blockStartDate: Date, weekIndex: number): { start: Date; end: Date } {
...
export function formatShort(d: Date): string { return SHORT.format(d); }
export function formatLong(d: Date): string { return LONG.format(d); }

export function formatRange(a: Date, b: Date): string {
  if (a.getFullYear() === b.getFullYear()) return `${SHORT.format(a)} – ${SHORT.format(b)}`;
...
export function weekIndexForDate(blockStartDate: Date, totalWeeks: number, d: Date): number | null {
  const start = new Date(blockStartDate); start.setHours(0,0,0,0);
