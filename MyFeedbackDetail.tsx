...
import { parseISODate, toISODate, formatLong } from "@/lib/coachdesk/periodization";
...
export function MyFeedbackDetail({
  client, session, onBack,
}: { client: { id: string; coach_id: string }; session: PlanSession; onBack: () => void }) {
  ...
  const [performedAt, setPerformedAt] = useState<string>(existing?.performed_at ?? session.planned_date);
  ...
  const { data: existingExLogsData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("exercise_logs")
        .select("id, session_exercise_id, weight_done, reps_done, notes, sets_json")
  ...
  function updateSet(exId: string, idx: number, patch: Partial<SetEntry>) {
    setForms((f) => {
      ...
      const nextSets = cur.sets.map((s, i) => (i === idx ? { ...s, ...patch } : s));
  ...
  function removeSet(exId: string, idx: number) {
    setForms((f) => {
      ...
      return { ...f, [exId]: { ...cur, sets: cur.sets.filter((_, i) => i !== idx) } };
    });
  }
    ...
    try {
      ...
      if (logId) {
        const { error } = await supabase
          .from("workout_logs")
          .update({ borg_scale: borg, overall_notes: overall, performed_at: performedAt })
          .eq("id", logId);
      ...
      if (existing?.id) {
        await supabase.from("exercise_logs").delete().eq("workout_log_id", existing.id);
      }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <Card className="p-4">
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          {performedAt !== session.planned_date && (
            <Button variant="ghost" size="sm" onClick={() => setPerformedAt(session.planned_date)}>
      ...
      {exs.map((e: any, i: number) => {
        const f = forms[e.id] ?? { sets: [], notes: "" };
        const weightSuffix =
          e.load_mode === "%1RM" ? "% 1RM" : e.load_mode === "bodyweight" ? "" : "kg";
        return (
          <Card key={e.id} className="p-4">
            ...
            <div className="mt-3 space-y-2">
              <div className="grid grid-cols-[auto_1fr_1fr_auto] items-center gap-2 text-xs text-muted-foreground">
                ...
                <span>Weight{weightSuffix ? ` (${weightSuffix.trim()})` : ""}</span>
                <span>Reps</span>
                <span className="w-8" />
              </div>
              {f.sets.map((s, idx) => (
                <div key={idx} className="grid grid-cols-[auto_1fr_1fr_auto] items-center gap-2">
                  <span className="w-10 text-sm font-medium text-muted-foreground">#{idx + 1}</span>
                  <Input
                    inputMode="decimal"
                    value={s.weight}
                    onChange={(ev) => updateSet(e.id, idx, { weight: ev.target.value })}
                  />
                  <Input
                    inputMode="numeric"
                    value={s.reps}
                    onChange={(ev) => updateSet(e.id, idx, { reps: ev.target.value })}
                  />
                  <Button
                    variant="ghost"
                    size="icon"
            ...
            <div className="mt-3">
              ...
              <Input
                ...
                onChange={(ev) =>
                  setForms((prev) => ({
                    ...prev,
                    [e.id]: { ...(prev[e.id] ?? { sets: [], notes: "" }), notes: ev.target.value },
                  }))
                }
              />
            </div>
