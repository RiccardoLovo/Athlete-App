...
export function AnalyticsPage() {
  const { clientId } = useParams({
    from: "/_authenticated/analytics/$clientId",
  });
    ...
    queryFn: async (): Promise<Row[]> => {
      ...
      for (const w of (data ?? []) as any[]) {
        ...
        for (const el of w.exercise_logs ?? []) {
          ...
          const prescribed =
            se.load_mode === "kg" && typeof se.load_value === "number"
              ? se.load_value
              : 0;
      ...
      <div className="mx-auto max-w-[1200px] p-6">
        ...
        {isLoading ? (
          ...
          <Card className="p-12 text-center text-muted-foreground">
            No completed workout data yet. Once the athlete logs feedback for
            sessions, progress will appear here.
          </Card>
        ) : (
...
function OverviewTab({ rows }: { rows: Row[] }) {
  const stats = useMemo(() => {
    ...
    const borgVals = Array.from(workouts)
      .map((id) => rows.find((r) => r.workoutLogId === id)!.borg)
      .filter((b) => b > 0);
    const avgBorg = borgVals.length
      ? borgVals.reduce((a, b) => a + b, 0) / borgVals.length
      : 0;
    ...
    const series = Array.from(dateMap.entries())
      ...
      .map(([d, v]) => ({
        ...
        borg: v.borgs.length
          ? v.borgs.reduce((a, b) => a + b, 0) / v.borgs.length
          : null,
      }));
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <StatCard
          label="Avg Borg"
          value={stats.avgBorg ? stats.avgBorg.toFixed(1) : "—"}
        />
        <StatCard
          label="Most improved"
          ...
          sub={
            stats.best
              ? `${stats.best.pct >= 0 ? "+" : ""}${stats.best.pct.toFixed(0)}%`
              : undefined
          }
        />
        <StatCard
          label="Sessions / week"
          value={stats.freq ? stats.freq.toFixed(1) : "—"}
        />
      </div>
...
function WeightTab({ rows }: { rows: Row[] }) {
  ...
  const exerciseOptions = useMemo(() => {
    ...
    for (const r of catFiltered) {
      const e = m.get(r.exerciseId) ?? {
        id: r.exerciseId,
        name: r.exerciseName,
        count: 0,
      };
      e.count++;
      m.set(r.exerciseId, e);
    }
  ...
  const selectedIds = useMemo(() => {
    if (exerciseId === "top5")
      return exerciseOptions.slice(0, 5).map((e) => e.id);
    return [exerciseId];
    ...
    for (const id of selectedIds) {
      ...
      if (reg)
        chartData.forEach((d, i) => {
          d[`${id}__trend`] = reg.slope * i + reg.intercept;
        });
    }
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          onClick={() => {
            const csv = toCsv(
              catFiltered
                .filter((r) => selectedIds.includes(r.exerciseId))
                .map((r) => ({
                  date: r.date,
          ...
          <LineChart data={chartData}>
            ...
            <Tooltip
              ...
              formatter={(value: any, key: any, item: any) => {
                ...
                if (String(key).endsWith("__trend"))
                  return [`${Number(value).toFixed(1)} kg`, `${name} (trend)`];
            ...
            <Legend
              formatter={(v: string) =>
                exNameById.get(v.replace("__trend", "")) ?? v
              }
            />
      ...
      <div className="mb-3 flex items-center gap-2">
        {warn && (
          <Badge className="bg-red-600 text-white">
            <AlertTriangle className="mr-1 h-3 w-3" /> High perceived exertion —
            consider adjusting load
          </Badge>
        )}
        <Button
          variant="outline"
...
function VolumeTab({ rows }: { rows: Row[] }) {
  ...
  const filtered = useMemo(
    () =>
      category === "all"
        ? rows
        : rows.filter((r) => r.category.includes(category)),
    [rows, category],
  );
    ...
    <Card className="p-4">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          size="sm"
          className="ml-auto"
          onClick={() => downloadCsv("volume.csv", toCsv(data))}
        >
          <Download className="mr-1 h-4 w-4" /> CSV
        </Button>
      </div>
