...
export function ExerciseBank({
  ...
  return (
    <aside
      className={`w-[320px] shrink-0 overflow-auto border-r bg-muted/30 p-3 ${className ?? ""}`}
    >
      ...
      <div className="mb-2 rounded border border-dashed border-primary/40 bg-primary/5 p-2 text-[10px] text-muted-foreground">
        <b>On iPad/touch:</b> tap an exercise below to select it, then tap a day
        to add it there.
      </div>
      <Input
        placeholder="Search…"
...
export async function insertExerciseIntoSession(
  ...
  const { data: ex } = await supabase
    .from("exercises")
    .select("discipline, stroke_default")
    .eq("id", exerciseId)
    .single();
...
export async function swapExerciseOrder(
  ...
  if (a.order_index === b.order_index) {
    await supabase
      .from("session_exercises")
      .update({ order_index: b.order_index + 1 } as never)
      .eq("id", b.id);
