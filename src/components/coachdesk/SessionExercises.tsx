...
export function SessionExercises({
  ...
  const { data: exs = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select(
          "*, exercises(name_en, discipline, stroke_default, structure_type, template_type, template_defaults)",
        )
  ...
  function bumpBlockCount(delta: number) {
    ...
    const caches = qc.getQueriesData<Record<string, number>>({
  ...
  return (
    ...
    >
      {exs.length === 0 ? (
        <div
          className={`rounded-md border border-dashed text-center text-xs text-muted-foreground ${compact ? "py-4" : "py-12"}`}
        >
          <Dumbbell
            className={`mx-auto mb-1 opacity-50 ${compact ? "h-4 w-4" : "h-6 w-6"}`}
          />
          Drag exercises here
        </div>
      ) : (
        exs.map((e, idx) => (
          <ExerciseCard
            ...
            se={e}
            defaultExpanded={
              !compact && idx === exs.length - 1 && exs.length <= 2
            }
