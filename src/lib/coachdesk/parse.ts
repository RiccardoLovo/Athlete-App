...
const DAY_RE =
  /^(monday|mon|tuesday|tue|tues|wednesday|wed|thursday|thu|thur|thurs|friday|fri|saturday|sat|sunday|sun)\b/i;
...
export function parseTextPlan(text: string): ParsedDay[] {
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      let slot = 1;
      if (/\b(pm|afternoon|evening|session\s*2|workout\s*2)\b/.test(rest))
        slot = 2;
      current = {
        day_number: dayNum,
    ...
    if (tempoColon) {
      tempo = tempoColon[1];
      s = s.replace(tempoColon[0], "").trim();
    }
    const tempoDash = s.match(/\b(\d-\d-\d-\d)\b/);
    if (!tempo && tempoDash) {
      tempo = tempoDash[1];
      s = s.replace(tempoDash[0], "").trim();
    }

    // Rest: "90s rest", "rest 90s", "2min rest", "rest 2min"
    let rest = "";
    const restMatch = s.match(
      /(?:rest\s+)?(\d+\s*(?:s|sec|secs|seconds|min|mins|minutes))\s*(?:rest)?/i,
    );
    ...
    if (wMatch) {
      weight = `${wMatch[1]}${wMatch[2].toLowerCase()}`;
    ...
    let sets = "",
      reps = "";
    const srMatch = s.match(
      /(\d+)\s*(?:x|×|\s+sets?\s+(?:x|of)\s*)\s*(\d+\s*(?:s|sec|secs|seconds|min|m)?)\s*(?:reps?)?/i,
    );
    if (srMatch) {
      sets = srMatch[1];
...
export function fuzzyMatchExercise(
  ...
  if (exact)
    return {
      exerciseId: exact.id,
      matchedName: exact.name_en,
      confidence: "high",
    };

  const contains = library.filter(
    (e) =>
      e.name_en.toLowerCase().includes(n) ||
      n.includes(e.name_en.toLowerCase()),
  );
  if (contains.length === 1) {
    return {
      exerciseId: contains[0].id,
      matchedName: contains[0].name_en,
      confidence: "high",
    };
  }
...
export function parseFeedbackText(text: string): ParsedFeedback {
  ...
  for (const line of lines) {
    ...
    if (name)
      result.exercises.push({ name, weight, reps, notes: notes || undefined });
  }
  return result;
}
